    ...
    def minimax(self, game, depth):
        ...
        **********************************************************************
        ...
        for move in game.get_legal_moves():
            v = max(best_score, self.min_value(game.forecast_move(move), depth, 1))
            if v > best_score:
                best_score = v
                best_move = move
        return best_move

    def min_value(self, game, depth, r_depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        ...
        for b in game.get_legal_moves():
            v = min(v, self.max_value(game.forecast_move(b), depth, r_depth+1))
        return v

    def max_value(self, game, depth, r_depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        ...
        v = float("-inf")
        for b in game.get_legal_moves():
            v = max(v, self.min_value(game.forecast_move(b), depth, r_depth+1))
        return v
